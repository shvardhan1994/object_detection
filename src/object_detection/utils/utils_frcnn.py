...
def prediction(data_test,model):
    """Function to perform inference on test dataset and postprocess predictions from MRCNN.

    Args:
        data_test (Dataset): Test dataset.
        model (model class): Pretrained MRCNN mode.

    Returns:
        GTBB (List): List of GT bounding boxes.
        PREDBB (List): List of respective MRCNN predicted bounding boxes.
        ...
        SCORE (List): List of scores obtained from MRCNN for each predicted bounding boxes.
    """
